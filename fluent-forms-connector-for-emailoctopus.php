<?php
/**
 * Plugin Name: Fluent Forms Connector for EmailOctopus
 * Plugin URI:  #
 * Description: Connect Fluent Forms with EmailOctopus.
 * Author: Tahmid ul Karim [WPManageNinja - Support]
 * Author URI:  https://profiles.wordpress.org/tahmidulkarim/
 * Version: 1.0.0
 * Text Domain: FFEMAILOCTOPUS
 */
...
define('FFEMAILOCTOPUS_DIR', plugin_dir_path(__FILE__));
define('FFEMAILOCTOPUS_URL', plugin_dir_url(__FILE__));
define('FFEMAILOCTOPUS_INT_KEY', 'EmailOctopus');

class FluentFormEmailOctopus
{
    ...
    {
        include_once FFEMAILOCTOPUS_DIR . 'Integrations/Bootstrap.php';
        include_once FFEMAILOCTOPUS_DIR . 'Integrations/API.php';
    }

    protected function registerHooks($fluentForm)
    {
       
       new \FluentFormEmailOctopus\Integrations\Bootstrap( $fluentForm );
    ...
    protected function injectDependency()
    {
        add_action('admin_notices', function () {
            ...
            $message = 'FluentForm EmailOctopus Add-On Requires Fluent Forms Add On Plugin, <b><a href="' . $pluginInfo->url
...
register_activation_hook(__FILE__, function () {
    ...
    $globalModules[FFEMAILOCTOPUS_INT_KEY] = 'yes';
    update_option('fluentform_global_modules_status', $globalModules);
});

add_action('plugins_loaded', function () {
    (new FluentFormEmailOctopus())->boot();
});
